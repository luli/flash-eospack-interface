!!****if* source/physics/Eos/EosMain/multiTemp/Eospac/Eos
!!
!!
!! NAME
!!
...
!==============================================================================
  ...
  use eos_eospacInterface, ONLY: eos_eospac
...
!!$  real,dimension(vecLen) :: gamcRow
  ...
  if(.not.present(massFrac)) then
     call Driver_abortFlash("[Eos] Eospac EOS implementation needs mass fractions")
  end if
...
!_RY:  print*, (maskPlus(midx), midx=1,EOS_NUM )
...
!!$  rowLen = ihi - ilo + 1
  ...
  if (ilo < 1 .OR. ilo > vecLen) then
     print*,'[Eospac/Eos] ilo is',ilo
     call Driver_abortFlash("[Eospac/Eos] invalid ilo")
  end if
  if (ihi < 1 .OR. ihi > vecLen) then
     print*,'[Eospac/Eos] ihi is',ihi
     call Driver_abortFlash("[Eospac/Eos] invalid ihi")
  end if
!!$  if (rowLen < 0 .OR. rowLen > vecLen) then
!!$     print*,'[Eospac/Eos] rowLen is',rowLen
!!$     call Driver_abortFlash("[Eospac/Eos] invalid rowLen")
!!$  end if
#endif
!!$  if (rowLen == 0) then
!!$     print*,'[Eospac/Eos] rowLen is 0.'
!!$  end if
...
#define ZpInv (1.0/Zp)
  ...
  case(MODE_DENS_EI_GATHER ) 
     useNRForAddtlCombined = .FALSE.; useNRForGamcCombined = .FALSE.
  case default
     call Logfile_stamp(mode,"[Eospac/Eos] mode not supported:&
                 should be one of [MODE_DENS_TEMP_GATHER, MODE_DENS_EI_GATHER]")
     call Driver_abortFlash("[Eospac/Eos] mode not supported:&
          should be one of [MODE_DENS_TEMP_GATHER, MODE_DENS_EI_GATHER]")
  end select
...
#define ggprod (eos_gasConstant/(eosData(gamc+ilo:gamc+ihi)-1.0))
  ...
  case (MODE_DENS_TEMP_GATHER)
     ...
     call eos_eospac(MODE_DENS_TEMP_ION,vecLen,eosData,ilo,ihi,massFrac,mask=maskPlus,componentMask=(/1,0,0/))
     ...
     call eos_eospac(MODE_DENS_TEMP_ELE,vecLen,eosData,ilo,ihi,massFrac,mask=maskPlus,componentMask=(/0,1,0/))
...
!!!$!!!        eosData(entr+ilo:entr+ihi)=stotRow(ilo:ihi) !(replace with expression based on component states)
  ...
  case (MODE_DENS_EI_GATHER)
     ...
     call eos_eospac(MODE_DENS_EI_ION,vecLen,eosData,ilo,ihi,massFrac,mask=maskPlus,componentMask=(/1,0,0/))
     gamIon(ilo:ihi) = eosData(gamc+ilo:gamc+ihi) !!  was gamcRow(ilo:ihi)
     call eos_eospac(MODE_DENS_EI_ELE,vecLen,eosData,ilo,ihi,massFrac,mask=maskPlus,componentMask=(/0,1,0/))
